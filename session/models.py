...
class Session(models.Model):
    ...
    def get_players(self):
        return self.players.filter(is_active=True).order_by(
            "-is_member",
            "-has_berkeley_email",
            "sign_up_date",
        )[: self.capacity]
    ...
    def get_wait_list(self):
        return self.players.filter(is_active=True).order_by(
            "-is_member", "-has_berkeley_email", "sign_up_date"
        )[self.capacity :]

    def get_queue(self):
        return self.queue.filter(is_active=True).order_by(
            "-is_member", "-has_berkeley_email", "date_joined"
        )[: self.capacity]
    ...
    def is_available(self):
        return (
            datetime.now() <= self.time - timedelta(minutes=cutoff)
    ...
    def is_training(self):
        return (
            self.session_type == "NT"
            or self.session_type == "IT"
            or self.session_type == "AT"
        )
